...
public class OAuth2AuthorizationEndpointFilter extends OncePerRequestFilter {
    ...
    private void processAuthorizationRequest(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        OAuth2AuthorizationRequestContext authorizationRequestContext = new OAuth2AuthorizationRequestContext(request.getRequestURL().toString(),
        ...
        OAuth2Authorization.Builder builder = OAuth2Authorization.withRegisteredClient(registeredClient).principalName(principal.getName())
        ...
        if (registeredClient.getClientSettings().requireUserConsent()) {
            ...
            Instant expiresAt = issuedAt.plus(5, ChronoUnit.MINUTES);        // TODO 应该可以配置授权码过期时间
    ...
    private void processUserConsent(HttpServletRequest request, HttpServletResponse response) throws IOException {
        UserConsentRequestContext userConsentRequestContext = new UserConsentRequestContext(request.getRequestURL().toString(),
                OAuth2EndpointUtils.getParameters(request));
        ...
        Instant issuedAt = Instant.now();
        Instant expiresAt = issuedAt.plus(5, ChronoUnit.MINUTES);        // TODO 应该可以配置授权码的过期时间
