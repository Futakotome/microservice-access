...
import io.futakotome.authService.oauth2.server.authorization.authorization.OAuth2AccessTokenAuthenticationToken;
import io.futakotome.authService.oauth2.server.authorization.authorization.OAuth2AuthorizationCodeAuthenticationToken;
import io.futakotome.authService.oauth2.server.authorization.authorization.OAuth2ClientCredentialsAuthenticationToken;
import io.futakotome.authService.oauth2.server.authorization.authorization.OAuth2RefreshTokenAuthenticationToken;
...
/**
 * 处理OAuth2 access token 请求
 * 默认请求 {@code /oauth2/token}方法{@code POST}
 * 默认请求可以覆盖{@link #OAuth2TokenEndpointFilter(AuthenticationManager, OAuth2AuthorizationService, String)}
 *
 * @author futakotome
...
public class OAuth2TokenEndpointFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            sendAccessTokenResponse(response, accessTokenAuthentication.getAccessToken(), accessTokenAuthentication.getIdToken(), accessTokenAuthentication.getRefreshToken());
        } catch (OAuth2AuthenticationException exception) {
            SecurityContextHolder.clearContext();
            exception.printStackTrace();
            sendErrorResponse(response, exception.getError());
        }
    }

    private void sendAccessTokenResponse(HttpServletResponse response, OAuth2AccessToken accessToken,
                                         OidcIdToken idToken, OAuth2RefreshToken refreshToken) throws IOException {
